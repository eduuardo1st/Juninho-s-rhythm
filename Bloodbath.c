...
void salvarPontuacao(int novaPontuacao) {
    const char *arquivoPontuacao = "resourses/pontuacao6.txt"; // Caminho do arquivo
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // música
    PlaySound("assets/Bloodbath.wav", NULL, SND_FILENAME | SND_ASYNC | 1);
    ...
    while (GetMessage(&msg, NULL, 0, 0)) {
        ...
        tempoAtual += 1; // Incrementa o tempo do jogo
    }
}
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        ...
        case WM_CLOSE:
            salvarPontuacao(pontuacao); // Salva a pontuação ao fechar
            DestroyWindow(hwnd);
            return 0;
    }
...
void MoverNotas(int tempoAtual) {
    ...
    if (!jogoEncerrado1 && pontuacao + erros == 20000) {
...
void DesenharPontuacao(HDC hdc) {
    ...
    SetTextColor(hdc, RGB(224, 30, 55));
