...
void salvarPontuacao(int novaPontuacao) {
    ...
    if (arquivo != NULL) {
        fprintf(arquivo, "Piratas do Caribe:   %d\n", pontuacaoFinal);
        fclose(arquivo);
    } else {
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    // música
    PlaySound("assets/piratas.wav", NULL, SND_FILENAME | SND_ASYNC | 1);
        ...
        case WM_PAINT:
            {
                ...
                FillRect(hdc, &ps.rcPaint, CreateSolidBrush(COR_FUNDO_FASE2)); 
...
void DesenharBarrasTeclas(HDC hdc) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        HBRUSH brush = CreateSolidBrush(COR_BOTOES_FASE2);
        SelectObject(hdc, brush);
...
COLORREF GerarCorPorColuna(int coluna) {
    switch (coluna) {
        case 0: // Primeira coluna
            return COR_NOTAS_1_FASE2; // Amarelo
        case 1: // Segunda coluna
            return  COR_NOTAS_2_FASE2; // Roxo
        case 2: // Terceira coluna
            return  COR_NOTAS_3_FASE2;     // Preto
        case 3: // Quarta coluna
            return  COR_NOTAS_4_FASE2;   // Azul
        default:
            return COR_NOTAS_1_FASE2; // Branco (ou outra cor padr�o)
    }
}
