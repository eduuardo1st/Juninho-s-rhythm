...
void salvarPontuacao(int novaPontuacao) {
    ...
    if (arquivo != NULL) {
        fprintf(arquivo, "Aqui Pensando:     %d\n", pontuacaoFinal);
        fclose(arquivo);
    } else {
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    // música
    PlaySound("assets/aquipensando.wav", NULL, SND_FILENAME | SND_ASYNC | 1);
...
void ReiniciarFase() {
    ...
    // Reinicia os eventos
    for (int i = 0; i < numeroEventos4; i++) {
 eventos4[i].jaCriada = 0; // Reseta o estado de cria��o dos eventos
    }
}
        ...
        case WM_PAINT:
            {
                ...
                FillRect(hdc, &ps.rcPaint, CreateSolidBrush(COR_FUNDO_FASE3)); 
...
void MoverNotas(int tempoAtual) {
    ...
    for (int i = 0; i < numeroEventos4; i++) {
        if (eventos4[i].tempo <= tempoAtual && eventos4[i].jaCriada == 0) {
            AdicionarNota(eventos4[i].tecla);
            eventos4[i].jaCriada = 1;
        }
    }
    ...
    // Verifica se a soma de erros e pontua��o � 
    if (!jogoEncerrado1 && pontuacao + erros == 465) {
...
COLORREF GerarCorPorColuna(int coluna) {
    switch (coluna) {
        case 0: // Primeira coluna
            return COR_NOTAS_1_FASE3;
        case 1: // Segunda coluna
            return COR_NOTAS_2_FASE3;
        case 2: // Terceira coluna
            return COR_NOTAS_3_FASE3;
        case 3: // Quarta coluna
            return COR_NOTAS_4_FASE3;
        default:
            return COR_NOTAS_1_FASE3; // Branco (ou outra cor padr�o)
    }
}
