...
void salvarPontuacao(int novaPontuacao) {
    ...
    if (arquivo != NULL) {
        fprintf(arquivo, "Megalovania(Undertale):    %d\n", pontuacaoFinal);
        fclose(arquivo);
    } else {
...
void ReiniciarFase() {
    ...
    // Reinicia os eventos
    for (i = 0; i < numeroEventos1; i++) {
 eventos1[i].jaCriada = 0; // Reseta o estado de cria��o dos eventos
    }
}
        ...
        case WM_PAINT:
            {
                ...
                FillRect(hdc, &ps.rcPaint, CreateSolidBrush(COR_FUNDO_FASE6)); 
...
void MoverNotas(int tempoAtual) {
    ...
    for (i = 0; i < numeroEventos1; i++) {
        if (eventos1[i].tempo <= tempoAtual && eventos1[i].jaCriada == 0) {
            AdicionarNota(eventos1[i].tecla);
    ...
    if (!jogoEncerrado1 && pontuacao + erros == 750) {
...
COLORREF GerarCorPorColuna(int coluna) {
    switch (coluna) {
        case 0: // Primeira coluna
            return  COR_NOTAS_1_FASE6;// Amarelo
        case 1: // Segunda coluna
            return COR_NOTAS_2_FASE6;// Roxo
        case 2: // Terceira coluna
            return COR_NOTAS_3_FASE6;// Preto
        case 3: // Quarta coluna
            return COR_NOTAS_4_FASE6;// Azul
        default:
            return COR_NOTAS_1_FASE6;// Branco (ou outra cor padr�o)
    }
}
